...
data _↓>>=_ {n β γ} {ρs : Level ^ n} {αψs : Level ²^ n} {Ψs : Effects ρs αψs}
            {B : Set β}  {Rs : Resources ρs} {Rs′ : B -> Resources ρs}
            (b : Eff Ψs B Rs Rs′) (C : B -> Set γ) : Set (β ⊔ γ) where
  call : (∀ y -> C y) -> b ↓>>= C

Call : ∀ {n β γ} {ρs : Level ^ n} {αψs : Level ²^ n} {Ψs : Effects ρs αψs}
         {B : Set β} {Rs : Resources ρs} {Rs′ : B -> Resources ρs} {b : Eff Ψs B Rs Rs′}
     -> (B -> Set γ) -> Set (β ⊔ γ)
Call {b = b} C = b ↓>>= C

⟨_⟩_ : ∀ {n β γ} {ρs : Level ^ n} {αψs : Level ²^ n} {B : Set β}
         {Rs : Resources ρs} {Rs′ : B -> Resources ρs}
     -> (Ψs : Effects ρs αψs) {b : Eff Ψs B Rs Rs′} -> (B -> Set γ) -> Set (β ⊔ γ)
⟨_⟩_ Ψs {b} C = b ↓>>= C
...
execᵈ : ∀ {n β γ} {ρs : Level ^ n} {αψs : Level ²^ n} {Ψs : Effects ρs αψs} {B : Set β}
          {Rs : Resources ρs} {Rs′ : B -> Resources ρs} {b : Eff Ψs B Rs Rs′} {C : B -> Set γ}
      -> (run : Eff Ψs B Rs Rs′ -> B) -> b ↓>>= C -> C (run b)
execᵈ run (call g) = g _

_<$>ᵈ_ : ∀ {n β γ δ} {ρs : Level ^ n} {αψs : Level ²^ n} {Ψs : Effects ρs αψs}
           {B : Set β} {Rs : Resources ρs} {Rs′ : B -> Resources ρs}
